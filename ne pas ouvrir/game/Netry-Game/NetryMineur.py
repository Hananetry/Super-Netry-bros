# Créé par Hana, le 23/03/2022 en Python 3.7
# DÃ©mineur
...
def affiche_nb_mines (nb_mines_voisines, col, lig):
    ...
    if tab_j[col, lig] == "":
        ...
        coul = ['grey','grey','grey','grey','grey','grey','grey']
...
def perdu():
    ...
    can.create_text((nb_col/2)*dim-15+gap, (nb_lig/2)*dim-5+gap,
                    text='Loser !', fill='black', font='Arial 50')
...
def gagne():
    can.create_text((nb_col/2)*dim-15+gap, (nb_lig/2)*dim-5+gap,
                    text='Congrats !', fill='black', font='Arial 50')
...
fen=Tk()
fen.title("Netry Miner Game - © 2022 Hana")
...
im_erreur = PhotoImage(file = "croixj.png")
im_flag = PhotoImage(file = "drapeauj.png")
...
case1.configure(text='Beginner', command=init_niveau, variable=choix,value=1)
case1.pack(anchor= NW ,padx=30)
case2=Radiobutton(f2)
case2.configure(text='Advanced', padx=3, command=init_niveau, variable=choix,value=2)
...
texte_mines = Label (f3, text = "Remaining mines :")
...
texte_cases = Label (f3, text = "Boxes to be processed:")
...
f1 = Frame(fen)
bou1 = Button(f1, width=14, text="New game", font="Arial 10", command=init_jeu)
bou1.pack(side=BOTTOM, padx=5, pady=5)
f1.pack(side=BOTTOM)

# Frame Ã  gauche de la grille de jeu pour afficher l'image
f4 = Frame(fen)
photo=PhotoImage(file="mine1.png")
